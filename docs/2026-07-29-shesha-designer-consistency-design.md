...
- **Missing assets:** `shesha-form-edit/references/archetypes.md` (cited as the definition of the eight archetypes; does not exist — only 1 of 8 archetypes has block coverage, and three incompatible archetype vocabularies are in use); `shesha-design-system/assets/block-styles/page-header-band.style.json`; `shesha-form-edit/test-log/` (breaks 3 `package.json` scripts); probe field `multiColumnContainers[].childWidths` (three docs depend on it; never emitted).
  > **Resolved, with one correction to the finding itself.** `archetypes.md` was written in Phase 1 (all 8 archetypes now have flow manifests); `test-log/`'s dead scripts were removed; the probe emits `childWidths` as of Phase 3. `page-header-band.style.json` was **never actually missing** — it was misfiled in a stray top-level `plugins/shesha-design-system/` directory that is not a plugin at all (no `plugin.json`, no skills), so nothing could reach it. Phase 5 moved it into place, merged a second misfiled overlay that had diverged from its in-place copy, deleted the stray directory, and added a guard test asserting every block's `$styleOverlay` resolves — the gap that let a misfiled overlay go unnoticed in the first place.
