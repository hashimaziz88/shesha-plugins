...
| 8b | **WP-3a.2** | The tier ladder: T1 schema tier (10 checks), T2 registry tier (22 checks), `verify.mjs` driver (D-098 split); registry completed for the used non-priority types (D-097) | §3.2.2, §3.2.3 | `node packages/verify/src/verify.mjs .build/wp3a --screen inline-editable-table --tiers t1,t2` → exit 0 |
| 8c | **WP-3a.3** | The coverage-proof layer: `g-exit-codes`, `g-mutation-coverage`, `g-defect-class-coverage`, `defect-classes.json`, the mutation-harness `kind:fixture` extension, and the tier fixtures (D-098 split) | §3.2.0, §3.5 | `node packages/verify/src/gates/g-mutation-coverage.mjs && node packages/verify/src/gates/g-exit-codes.mjs && node packages/verify/src/gates/g-defect-class-coverage.mjs` → exit 0 |
