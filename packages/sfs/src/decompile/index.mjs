...
function nodeKindOf(reg, mk) {
  ...
  // dropdown/autocomplete are the two SFS `select` targets (§2.1.4); lifting them
  // to `select` (not field+raw) makes the recompile take the identical emit path,
  // so the type-specific key order matches byte-for-byte.
  if (type === 'dropdown' || type === 'autocomplete') return 'select';
...
function inferKind(components, formSettings, diagnostics) {
  ...
  // Invert the kind->formSettings signature table (§2.1.3). Each kind emits a
  // distinct (dataLoaderType, dataSubmitterType) pair, so the emitted settings
  // recover a kind that reproduces the SAME settings — which is what round-trip
  // needs. modal and create share (none, gql); dashboard and custom share
  // (none, none); the ambiguous member is chosen so the settings are identical.
  const loader = formSettings.dataLoaderType;
  const submitter = formSettings.dataSubmitterType;
  // A table wins over the submitter signature: a list carrying a submitter is the
  // N8 defect, normalised away, not an `edit` form. Check hasTable first.
  if (hasTable && loader === 'gql') return 'list';
  if (loader === 'gql' && submitter === 'none') return 'detail';
  if (loader === 'none' && submitter === 'gql') return 'create';
  if (loader === 'gql' && submitter === 'gql') return 'edit';
  if (loader !== 'none' || submitter !== 'none') {
    diagnostics.push({ severity: 'info', code: 'DEC-7201', message: `unmatched formSettings signature (${loader}/${submitter}); defaulted to custom` });
  }
  return 'custom';
}
  ...
  const lift = (mk, i, parentPath, taken) => {
    ...
    if (kindOf === null) {
      ...
      if (rec !== undefined && rec.isInput === true) {
        // The field variant requires `bind`; it is the emitted propertyName. Any
        // dropdown/autocomplete data-source props the field grammar cannot express
        // (referenceListId, dataSourceType, …) are carried by the residue pass, so
        // this stays round-trip-lossless — which is why the field is registered in
        // `sources` here just like a container-backed region.
        const bind = typeof mk.propertyName === 'string' ? camelPath(mk.propertyName) : name;
        sources.set(`${parentPath}/${name}`, mk);
        return { node: 'field', name, bind, component: String(mk.type) };
      }
      structuralEscapes += 1;
      return {
        node: 'raw',
    ...
    if (kindOf === 'text') {
      ...
      // regenerated by the token-driven recipe or carried by the residue pass.
      if (typeof mk.content === 'string') region.content = mk.content;
    }
    if (kindOf === 'status') {
      // The status variant requires refList; recover it from the emitted
      // referenceListId:{module,name}, and the binding from propertyName.
      const ref = isObj(mk.referenceListId) ? mk.referenceListId : undefined;
      if (ref !== undefined) region.refList = `${String(ref.module)}/${String(ref.name)}`;
      if (typeof mk.propertyName === 'string') region.bind = camelPath(mk.propertyName);
      if (mk.solidBackground === true) region.solid = true;
      if (mk.showReflistName === false) region.showName = false;
      if (mk.showIcon === true) region.showIcon = true;
    }
    if (kindOf === 'list') {
      // The list variant requires rowTemplate; recover it from the emitted formId.
      const fid = isObj(mk.formId) ? mk.formId : undefined;
      if (fid !== undefined) region.rowTemplate = `${String(fid.module)}/${String(fid.name)}`;
    }
    if (kindOf === 'select') {
      // The select variant requires bind; recover the binding, the data source
      // (entitiesList -> entity, else the referenceList default), and the ref list.
      if (typeof mk.propertyName === 'string') region.bind = camelPath(mk.propertyName);
      if (mk.dataSourceType === 'entitiesList') region.source = 'entity';
      const ref = isObj(mk.referenceListId) ? mk.referenceListId : undefined;
      if (ref !== undefined) region.refList = `${String(ref.module)}/${String(ref.name)}`;
    }
...
function liftedName(mk, kindOf, i) {
  // The region `name` is the emitted componentName (§2.1.4); prefer it so the
  // recompiled node keeps the same name -> same seeded id. Fall back to the
  // camelCased propertyName, then a positional synthesis. componentName is only
  // used when it is a legal SFS name.
  const NAME = /^[a-z][A-Za-z0-9]{0,39}$/;
  if (typeof mk.componentName === 'string' && NAME.test(mk.componentName)) return mk.componentName;
  if (typeof mk.propertyName === 'string' && mk.propertyName.length > 0) {
    const c = camelPath(mk.propertyName);
    if (NAME.test(c)) return c;
  }
