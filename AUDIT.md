...
| Item | Why it is not a document fix | Proposed owner |
|---|---|---|
| `g-oracle-independence` does not exist, so D-076 and D-077 stand on `pending:WP-1.a` | Writing the gate is code | WP-1.a, before it may be recorded complete |
| The brief bundle at 604,699 B against 61,440 B | Extracting tables and literals is a work package | BL-011 |
| CONTROL and the five sections sit outside `g-commands-executable`'s reach | Widening a gate's declared subject changes what green means | BL-011, which is already the row that restructures the brief |
