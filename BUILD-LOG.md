...
## WP-8b.1 — Hooks foundation + block-form-writes (operating layer, D-118) — 2026-08-24

Status: complete
Gate: `node --test packages/verify/test/hooks.test.mjs` -> `# pass 18 # fail 0` (12 matcher cases + 16 block-form-writes decide cases + `p95 <= 5ms`); `node packages/verify/src/gates/g-hook-contract.mjs` -> exit 0 (3 families); `npm run green` -> exit 0 (33 gates)
Evidence: packages/verify/evidence/WP-8b.1.json
Decisions added: D-119 (hooks exit only 0/2, exit(1) banned, runner/decide split; enforcer g-hook-contract)
Blocked: none new
Next: WP-8b.2 (enforce-screen-lock + validate-sfs-on-write hooks + the sfs validate CLI)

WP-8b.1 lays the hook foundation and the first hook. `.claude/hooks/lib.mjs` carries the zero-dependency primitives every hook imports — `findRepoRoot` (walks up for the `shesha-plugins` package.json, **fails closed** to null → HOOK-0001), `matchGlob` (anchored; `*` never crosses `/`, `**` does, `?` throws), `payloadPaths`/`bashPaths` (the §4.3.3 extraction table), `openLocks`, `logDecision` (a logging failure never changes the decision), and `emit` (exit 0 with the decision, or exit 2 if stdout is unwritable — never exit 1). `block-form-writes.decide.mjs` is a **pure** `decide(payload, ctx)` implementing R0–R7 first-match-wins: R1 denies a `Write` of `*.form.json`/`*.expected.form.json` (HOOK-0101); R2 denies a `Bash` redirect/writeFile to markup unless the command is on the writer allowlist (`npm run <pkg-script>` or `node packages/{sfs,mcp,verify}/**`, computed from the root package.json, not hard-coded); R3 protects the toolchain-only files; R4 is Specwriter markup-blindness; R5 is the run-dir writable-set allowlist; R6 is Evaluator log-blindness. `block-form-writes.mjs` is the thin runner (stdin → ctx → decide → log → emit). `hooks.test.mjs` runs `decide` **in-process** against temp roots — no child, no cold start — proving all 16 R-rule cases (a–p from §4.3.3), the 12 matcher cases, and a p95 ≤ 5 ms over 500 non-matching calls. `g-hook-contract` (2 mutations) bans `exit(1)` in every hook file, checks every hook wired in `.claude/settings.json` exists, and checks every `<name>.decide.mjs` has a `<name>.mjs` runner. The gate-ratchet floor rises 32 → 33; `.claude/settings.json` wires `block-form-writes` on PreToolUse; `prove-b` gains its `hook contract` step. The remaining five hooks join in WP-8b.2/8b.3.


## WP-8b.2 — enforce-screen-lock + validate-sfs-on-write + `sfs validate` CLI (operating layer, D-118) — 2026-08-24

Status: complete
Gate: `node --test packages/verify/test/hooks.test.mjs` -> `# pass 34 # fail 0` (12 matcher + 16 block-form-writes + 9 enforce-screen-lock + 7 validate-sfs-on-write cases + `p95 <= 5ms`); `node packages/verify/src/gates/g-hook-contract.mjs` -> exit 0; `npm run green` -> exit 0 (33 gates)
Evidence: packages/verify/evidence/WP-8b.2.json
Decisions added: D-120 (`sfs validate` CLI + the two write-path hooks; enforcer g-hook-contract)
Blocked: none new
Next: WP-8b.3 (gate-push + gate-dispatch + session-start hooks + push-admissible + g-hook-liveness + the `sfs run` CLI)

WP-8b.2 adds the `sfs validate` subcommand and the two write-path hooks that use it. `sfs validate --schema <name> --file <path> [--json]` compiles one of the `packages/sfs/schema/*.schema.json` handoff schemas under ajv and validates a JSON file against it — exit 0 valid · 1 invalid · 2 usage, **writing nothing** (INV 1 holds: the validator only reads). `--json` prints `{ok, diagnostics}` so a hook can surface the diagnostics verbatim. `enforce-screen-lock.decide.mjs` (PreToolUse on Write/Edit/NotebookEdit) is the **fan-out mutex plus contract precondition**: L1 requires a `planner` lock to write `plan.json` (HOOK-0401); L2 denies a screen write with no lock (HOOK-0402); L3/L4 deny a screen held by another `sfs-specwriter` — fresh (HOOK-0403) or stale >30 min (HOOK-0404, released explicitly, never auto-stolen); L5 denies implementation before a signed acceptance contract with ≥3 predicates exists in `plan.json` (HOOK-0405); L6 is the 3-round repair cap (HOOK-0406). Its lock identity is `(role, screen, runId)` — **no session_id**. `validate-sfs-on-write.decide.mjs` (PostToolUse) spawns `sfs validate` for `*.sfs.json`/`*.sfs.meta.json`/`plan.json`/`dispatch/*.json`: V1 blocks HOOK-0002 if the validator will not start (never a silent pass), V2 allows a valid file, V3 blocks with the verbatim diagnostics **and renames the file to `*.rejected`** (a PostToolUse block does not undo the write, so an SFS-1002 input must not stay on disk), and V4 blocks HOOK-0203 when a valid SFS's `form`/`module` disagree with its `plan.json` row. Both decides are **pure over ctx** (`{root, fs}` and `{root, fs, spawnNode}`), so `hooks.test.mjs` runs them in-process against temp roots with a stubbed `spawnNode` — 16 new cases (9 + 7). `g-hook-contract` discovers the new hooks automatically (it walks `.claude/hooks` and reads `.claude/settings.json`): both runners are exit-0/2 clean, both are wired (enforce-screen-lock on PreToolUse, validate-sfs-on-write on PostToolUse), and both decides have runners. `prove-b` gains its `sfs validate CLI` step. WP-8b.3 adds the remaining three hooks and the `sfs run` CLI.

## WP-8b.3 — gate-push + gate-dispatch + session-start + push-admissible + g-hook-liveness + `sfs run` (operating layer, D-118) — 2026-08-24

Status: complete
Gate: `node --test packages/verify/test/hooks.test.mjs` -> `# pass 47 # fail 0` (12 matcher + 16 block-form-writes + 9 enforce-screen-lock + 7 validate-sfs-on-write + 4 gate-push + 7 gate-dispatch + 2 session-start + p95); `node --test packages/verify/test/push-admissible.test.mjs` -> `# pass 14 # fail 0` (P0-P9 + P5a refusals + green admission + T3-partial); `node packages/verify/src/gates/g-hook-liveness.mjs` -> exit 0; `npm run green` -> exit 0 (34 gates)
Evidence: packages/verify/evidence/WP-8b.3.json
Decisions added: D-121 (push admission is one program P0-P9; the three hooks; sfs run; enforcer g-hook-liveness)
Blocked: none new
Next: WP-8c (3 agent files via git mv + settings enable + g-agent-contract/g-specwriter-purity/g-judge-isolation/g-plugin-enabled/g-markup-provenance)

WP-8b.3 completes the six-hook operating layer. Push admission is one program, not hook logic: `packages/verify/src/bin/push-admissible.mjs` imports `coverage.mjs` and evaluates P0-P9 in order, first failure refuses (exit 0 admissible / 1 refused / 2 usage). `pushAdmissible(verdict, {allowPartial, present})` in `coverage.mjs` is the single T1-T3 rule replacing three: T1 pass; T2 pass or partial where every uninspectable pointer's component type is absent from the artifact; T3 pass or partial only with `--allow-partial` and every uninspectable reason a backend/metadata outage. `--allow-partial` is the one escape - explicit, per-invocation, logged; there is no config flag that turns off P0-P9. `push-admissible.test.mjs` proves one refusal per condition (P0-P9 + P5a) plus a fully-green admission (14 cases). `gate-push.decide.mjs` (PreToolUse) detects a push - the sfs push verb, a bare run-push, or a raw curl to the markup/FormConfiguration endpoints (the obvious bypass) - spawns the admission program and maps its exit; a non-push never spawns; a program that cannot start denies HOOK-0002. `gate-dispatch.decide.mjs` (PreToolUse on Task) enforces D0-D6: a subagent is dispatched by a `dispatch/<role>-<screen>-r<n>.json` path (HOOK-0501), which must validate against `dispatch.schema.json` (HOOK-0502); a judge is never handed a logs/rationale/self-report path (HOOK-0503, judge isolation, D-016); handoffs carry paths not contents (HOOK-0504); one screen per specwriter (HOOK-0505). `session-start.decide.mjs` (SessionStart) prints exactly six ground-truth lines as additionalContext and never blocks - a blocked SessionStart makes the repo unopenable; line 2 spawns `green-quick.mjs` (typecheck + gates, 20 s cap) and is "not measured (timeout)" on failure, never a verdict. `sfs run lock/release/new` manages the fan-out locks the hooks reference. `g-hook-liveness` (2 mutations) fails when a run's `hooks.jsonl` has fewer lines than its `manifest.json` events - a hook that stopped firing is a silent loss of the audit trail - over the committed fixture run at `packages/verify/test/fixtures/run/`. The gate-ratchet floor rises 33 -> 34; `settings.json` wires all six hooks; `prove-b` gains its push-admission step. WP-8b is complete.

## WP-8c.1 — six agent files + g-agent-contract/g-judge-isolation/g-plugin-enabled + plugin enable (operating layer, D-122/D-123) — 2026-08-24

Status: complete
Gate: `node packages/verify/src/gates/g-agent-contract.mjs` -> exit 0 (frontmatter 6, roles 3); `node packages/verify/src/gates/g-judge-isolation.mjs` -> exit 0 (no-leak 591, schema-pattern 1, evaluator-tools 1); `node packages/verify/src/gates/g-plugin-enabled.mjs` -> exit 0; `npm run green` -> exit 0 (37 gates)
Evidence: packages/verify/evidence/WP-8c.1.json
Decisions added: D-122 (WP-8c split into 8c.1/8c.2), D-123 (agent layer + 3 gates; enforcer g-agent-contract)
Blocked: none new
Next: WP-8c.2 (g-markup-provenance closing D-003 + g-specwriter-purity + fixtures + delete clean-form-config skill)

WP-8c.1 makes the run harness's agents real files, each proved by a gate rather than by a rule addressed to a model's memory. `sfs-specwriter.md` (`git mv` of `form-author.md`) and `sfs-evaluator.md` (`git mv` of `form-auditor.md`) get entirely new bodies; `sfs-planner.md` is new; `design-critic.md`, `fleet-transformer.md` and `fullstack-prereq-checker.md` are rewritten (design-critic to `tools: Read` only and under the 4096 B cap it was over, fleet-transformer to the SFS-transform unit of work, fullstack-prereq-checker to the `$RUN_DIR/prereq/<entity>.json` output contract). Each body is a numbered procedure plus a fixed return block — no `## Hard stops`, no restatement of a hook rule or schema constraint, because a denial arrives from the hook with its own corrective command. `g-agent-contract` (2 mutations) walks all six: `name` == basename, `tools`/`disallowedTools` present and disjoint, no `MultiEdit`, <= 4096 B, no `## Hard stops`; plus the role restrictions (specwriter/evaluator deny Bash + push, design-critic tools is exactly Read). `g-judge-isolation` (2 mutations) keeps the builder self-report token more than 400 chars from any evaluator/critic reference across plugins/** and packages/** (excluding the schema that defines the rule), pins `dispatch.schema.json`'s `not.pattern` to the canonical isolation regex, and bars Bash from the evaluator — the gate assembles the forbidden token at runtime so its own source never trips its scan. `g-plugin-enabled` (2 mutations) requires `.claude/settings.json` `enabledPlugins` to enable `shesha-developer` (an empty map leaves `Task(subagent_type)` unresolved). `packages/mcp/src/index.mjs` now declares the seven-tool L4 name surface the agents reference (the server runtime is WP-8d); `plugin.json` bumps 1.9.3 -> 1.10.0; the gate-ratchet floor rises 34 -> 37. The clean-form-config deletion moved to WP-8c.2 (D-122): its `assets/groups/*.json` is the component index `g-check-references` reads, so its removal is an asset migration, not an agent-layer change.

## WP-8c.2 — g-markup-provenance (INV 1) + g-specwriter-purity + clean-form-config deletion (operating layer, D-124/D-125) — 2026-08-24

Status: complete
Gate: `node packages/verify/src/gates/g-markup-provenance.mjs` -> exit 0 (16 artifacts recompile byte-identically); `node packages/verify/src/gates/g-specwriter-purity.mjs` -> exit 0; `node packages/verify/src/gates/g-check-references.mjs` -> exit 0 (index read from packages/registry/data/prop-groups); `npm run green` -> exit 0 (39 gates)
Evidence: packages/verify/evidence/WP-8c.2.json
Decisions added: D-124 (g-markup-provenance closes D-003; clean-form-config deletion), D-125 (g-specwriter-purity); D-003 and D-016 enforcers closed from pending:BL-008 to g-markup-provenance / g-judge-isolation
Blocked: none new
Next: WP-8d (packages/mcp server + 3 transports + g-mcp-cli-parity + transports.test + .mcp.json)

WP-8c.2 gives INV 1 its real enforcer. `g-markup-provenance` (3 mutations) is the decision procedure a command-line matcher cannot be: for every compiled artifact — the 13 `packages/sfs/test/fixtures/clean/*.expected.form.json`, the fixture run's `screens/employees.form.json`, and any `runs/**/screens/*.form.json` — it locates the sibling `*.sfs.json` and the sibling `*.compiled.meta.json` (registrySha256, brand, compilerVersion), recompiles the SFS under exactly those, and hard-fails unless the emitted bytes are byte-identical (D-021 makes this decidable). A subject with no sibling SFS, or a meta missing a hash, is a hard failure, never uninspectable — that is what catches a computed-path writer the hook allowed (mutation c). `registryFingerprint(root, brand)` in `registry.mjs` is CRLF/BOM-normalised and shared by the fixture generator and the gate, and the fixtures are LF-pinned in `.gitattributes` so `core.autocrlf` cannot break byte-identity. `g-specwriter-purity` (2 mutations) scans every `logs/specwriter-*.md` under the fixture run and `runs/**` and hard-fails on any of the six markup fingerprints; `packages/verify/test/fixtures/run-dirty/` is the committed negative control the mutation points it at. **clean-form-config is deleted as a skill** — a MUST-strength every-push gate that shipped zero scripts (§1.7 T8): its eight `assets/groups/*.json` move to `packages/registry/data/prop-groups/`, `g-check-references` reads the index there now, `command-floor.json`'s dead-command waiver and the `handoff-contract.md` reference are removed, and `disposition.json` records all 13 rows. `D-003` and `D-016`, pending since WP-0, close to their live gates. The gate-ratchet floor rises 37 -> 39; `prove-b` gains its `markup provenance` step. WP-8c is complete.

## WP-8d — the shesha-sfs MCP surface: 7 tools, 3 transports, CLI parity (operating layer, D-126) — 2026-08-24

Status: complete
Gate: `node --test packages/mcp/test/transports.test.mjs packages/mcp/test/grammar.test.mjs` -> `# pass 7 # fail 0` (ESM 7 tools · ajv-strict schemas · stdio tools/list=7 · --host 0.0.0.0 exit 2 · http registry_lookup · CLI parity · GBNF header); `node packages/verify/src/gates/g-mcp-cli-parity.mjs --json` -> `{"result":"pass","tools":7,"subcommands":7,"schemaMismatches":0}`; `npm run green` -> exit 0 (40 gates)
Evidence: packages/verify/evidence/WP-8d.json
Decisions added: D-126 (the MCP surface; enforcer g-mcp-cli-parity)
Blocked: none new
Next: WP-3c (T4 smoke tier + layout-probe ESM port), WP-3d (T5 advisory)

WP-8d builds the `shesha-sfs` MCP surface as **one implementation behind three adapters**. `packages/mcp/src/tools/*.mjs` is the single implementation of the seven tools — `compile` (the only writer of `*.form.json`), `decompile`, `verify` (the only writer of `*.verdict.json`), `registry_lookup` (exact, never fuzzy), `metadata_entity` (source `none` with no backend, never an empty `live`), `precedent_search` (shape retrieval, never correctness), and `push` (re-runs P0–P9 in the tool, the authority the hook pre-checks). `packages/mcp/src/tools/index.mjs` exports the table as importable ESM so an agent with `Bash` batches calls in a script. `packages/mcp/bin/server.mjs` is the three adapters: `--stdio` (JSON-RPC via `@modelcontextprotocol/sdk` 1.30.0, the one external dependency, exact-pinned), `--http --host 127.0.0.1` (loopback only — any other `--host` exits 2, never a silent `0.0.0.0` bind), and a CLI subcommand per tool whose flag set is derived from each tool's `inputSchema` so the two surfaces cannot drift. `/.mcp.json` launches the stdio server. `sfs registry <type>` gives the CLI the same exact answer as `registry_lookup` without a dependency on `packages/mcp` (both call `loadRegistry`), and `sfs registry grammar` emits a GBNF whose header states constrained decoding applies only to the emission step, never a reasoning step. `g-mcp-cli-parity` (2 mutations) asserts the exported tool set equals the CLI subcommand set, the flag sets are schema-driven, and the ESM export exists; `transports.test.mjs` proves the three transports (6 cases) and `grammar.test.mjs` the header. The gate-ratchet floor rises 39 -> 40; `prove-b` gains its `mcp tool surface` step. WP-8 is complete.

## WP-3c — T4 live smoke and T4b DOM residue: the last two ladder tiers (D-127) — 2026-08-28

Status: complete
Gate: `node --test packages/verify/test/probe.test.mjs packages/verify/test/t4b-residue.test.mjs packages/verify/test/t4-smoke.test.mjs` -> `# pass 28 # fail 0`, printing `data-sha-c-name captured on 17/17 named nodes · summary bytes 8139 <= 8192 · rowBand parent-relative true`; `node packages/verify/src/tiers/t4-smoke.mjs --selftest --json` -> exit 0; `node packages/verify/src/tiers/t4b-residue.mjs packages/sfs/test/fixtures/probe/login.probe.overflow.json --json` -> exit 1 and `...no-name.json` -> exit 3; `node packages/verify/src/verify.mjs .build/wp3c --screen inline-editable-table --tiers t1,t2,t3,t4 --metadata packages/sfs/test/fixtures/metadata/inline-editable-table.metadata.json --json` -> exit 3 with `result: pass` and `tiers.T4 = {"result":"notRun","reason":"no --base-url given"}`; `npm run green` -> exit 0
Evidence: packages/verify/evidence/WP-3c.json
Decisions added: D-127 (T4/T4b disposal, tier location and the T4b.02 rounding rule; enforcer g-blocked-honesty)
Blocked: B14 — the live T4 path (clicking every action site against a real host) is BL-033
Next: WP-3d (T5 advisory visual)

WP-3c splits the live tier along the only line that makes it testable offline: `capture()` DRIVES a page and RECORDS what happened, `t4Smoke()` ASSERTS over that recording. Only the recording needs a browser, so `--selftest` proves all six checks against `packages/verify/test/helpers/stub-backend.mjs` (a `node:http` ABP stub covering 403, non-JSON and `result: null`) and `npm run green` launches nothing. Absent Playwright or `--base-url`, T4 reports `notRun` with a reason **assembled from what is actually missing** — it never claims an installed package is absent. T4b reports inside `tiers.T4`, because the verdict envelope is `additionalProperties:false` over T1..T5 and there is no legal T4b key. The five probe fixtures are **recorded from the running Shesha frontend**: 98 measured nodes, 17 carrying `data-sha-c-name`, with the four defect probes minimal subtrees of that same capture carrying one recorded geometry edit each.

An adversarial pass over the first draft broke it four ways, and every hole it found is now a mutation rather than a comment. **Attribution no longer skips anything**: the draft disposed a node `notApplicable` when no `data-sha-c-name` owned it, and because a notApplicable note is never printed, stripping ONE ancestor's attribute turned a 500px clip and a 23,880px² overlap into a silent pass — every node is now measured, a missing name costs only the handle back to a component, and the one genuine uninspectable left is that a probe naming nothing cannot be confirmed to be of the intended screen. **Capabilities are validated, not trusted**: a probe declaring `{"geometry": false}` used to downgrade every real defect to uninspectable, so a key the instrument does not define is now a hard failure — an escape hatch that downgrades failures is banned behaviour T14, and the mutation that used to certify it now catches it. **Overlap is no longer limited to DOM siblings**: two named controls in different subtrees painted exactly on top of each other passed, which is precisely the columns-collide class the check exists for. **Sub-pixel geometry is preserved**: `Math.round` on viewport rects made two abutting boxes report a full-width one-pixel intersection, so the draft had to ignore any overlap one pixel deep — and therefore could not see a genuine 1440×1 double-paint. Rects are recorded to 2dp, the floor is 0.1px, and exact abutment now intersects by zero. Only a deliberate `z-index` excuses an out-of-flow node covering its neighbours.

Four latent defects are fixed in passing, three of them outside this WP's own code. `verify.mjs` accepted any tier name: `result` starts at `pass` and is only ever raised, so `--tiers banana` sealed a verdict claiming `pass` having run nothing — unknown names are now refused. The **MCP `verify` tool** — the one an agent calls — defaulted to `['T1','T2','T3']` while the ladder matches lower case, so it matched no tier and returned that same green verdict over nothing; it lower-cases now and returns the exit code it used to discard. The verdict schema's tier `result` enum lacked `notRun` while the frozen `RESULTS` vocabulary requires it, so a real run's verdict would have been refused by `push-admissible`. And a probe recorded from one screen satisfied every other screen's T4. `BLOCKED.md` promised a `g-blocked-honesty` cross-check that shipped no code (§1.7 T8) — it ships here, and requires a row's degraded state to CITE a live tier, gate or config in a code span rather than merely mention one. `quarantine/` is gone: its last file, the CJS DOM probe, is superseded by `packages/verify/src/probe/layout-probe.mjs` carrying all six §3.2.5 changes. The gate-ratchet floor rises 40 -> 41; `prove-b` gains its `T4/T4b residue` step.

## WP-3d — T5 visual advisory: a ranking signal that can never become a gate (D-128) — 2026-08-28

Status: complete
Gate: `node --test packages/verify/test/t5.test.mjs` -> `# pass 10 # fail 0` including `verdict.result is byte-identical whether t5 reports the best or the worst possible score vector`; `node packages/verify/src/tiers/t5-visual.mjs --selftest --json` -> exit 0 on every grade including the worst; `node packages/verify/src/gates/g-t5-advisory.mjs` -> exit 0 printing `t5 references in gate-push.mjs: 0 · t5 references in result computation: 0 · judge-truth gap recorded for 1.11.0`; `npm run green` -> exit 0
Evidence: packages/verify/evidence/WP-3d.json
Decisions added: D-128 (the T5 contract; enforcer g-t5-advisory). D-015 closes from `pending:BL-005` to `g-t5-advisory`
Blocked: B15 — no judge model is qualified, so T5 reports notRun for every screen (BL-034)
Next: the Scope-B close-out — `prove-b --bless`, then `SESSION COMPLETE — SCOPE B`
