...
5. **Verify** — three gates in order: **structural** (native components, fully flexed, fields bound) → **placement diff** (`shesha-design-comprehension` re-measures the live form against the blueprint assertions) → **visual** (screenshot vs theme). Mismatches route back to the owning skill. **On 0.43:** forms are versioned ConfigurationItems, so each fix-iteration re-pushes the **same in-flight Draft** (`CreateNewVersion` once per session) and publishes to Live only when all gates pass; and after every create/publish the adminportal **IndexedDB caches (`forms`/`entities`/`ref-lists`/`misc`) must be cleared before re-probing**, or the probe measures a stale/404ing form. See [`../shesha-form-edit/references/version-lifecycle.md`](../shesha-form-edit/references/version-lifecycle.md).
...
- `$antdTheme` *(default brand)* — the pre-resolved Ant Design `ConfigProvider` `{token, components}` object for **this 0.43 backend's Ant version**, applied verbatim at the app level (the "set once" theme layer)
...
[`shesha-design-system/assets/capability-matrix.json`](../shesha-design-system/assets/capability-matrix.json) (+ a readable `.md`) records which v7 style channel actually **renders** on which component, measured against a live backend. In this 0.43 plugin the matrix and the `$antdTheme` it pairs with are the **0.43-measured** set (component v7 style-channel support and the Ant version differ on a 0.43 backend). It is the source of truth for "what works" and gets **re-measured on a Shesha upgrade** (diff = upgrade-impact report). `validate-blocks.js` gates every block against it.
...
- **One push path** (form-edit). Theme is set **once** — `shesha-design-system` applies the token file's app-level theme via the legacy `/settings` pages / the settings + FormConfiguration REST API — not per-form, and not by editing frontend source.
