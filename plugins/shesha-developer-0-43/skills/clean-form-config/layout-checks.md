...
**Rule:** Both components require a data-wrapper ancestor (the wrapper that fetches the rows) in order to receive data. On a 0.43 backend the data-wrapper component type is `datatableContext` (there is no `dataContext` component on 0.43). A `datatable` or `datalist` placed at the root level, or nested only inside layout containers (card, panel, container, columns, tabs, etc.), will render but show no data.

**How to check:**

1. Walk the component's ancestor chain (via `parentId` lookups) up to the form root.
2. At each ancestor, check `ancestor.type`.
3. Pass if **any** ancestor has `type === "datatableContext"` (also accept the legacy `type === "dataContext"` so forms migrated from a 0.45 backend are not flagged spuriously).
...
*(Future checks — e.g. minWidth > maxWidth, height: "100%" inside auto parent — append here as L6, L7, etc.)*
