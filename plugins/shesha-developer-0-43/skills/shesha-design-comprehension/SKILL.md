...
4. **Hand the blueprint to `shesha-form-edit`** as the build's requirements (archetype + seed selection + column spans + per-field binding). **REQUIRED PARTNER:** `shesha-developer-0-43:shesha-form-edit` builds the structure.
...
| Concern | Skill |
|---|---|
| Ingest design, plan screens, orchestrate, verify end-to-end | `shesha-developer-0-43:shesha-claude-designer` (calls this skill per screen) |
