...
- Channel 4 is the legacy style-string leftover. `style` can also exist *inside* breakpoints (`desktop.style`) — check both.
...
- Container css-in-js is hard-coded `outer { overflow: hidden }`, `inner { ...getOverflowStyle(true,false) }` → every breakpoint-block container's **inner div is ALWAYS `overflow:auto`** (this holds for the 0.43 `container` at v6). The markup `overflow` prop is a **no-op in view mode** — stamping `overflow:false` does nothing.
...
2. When migrating a legacy style-string component to breakpoint-block props (at its live 0.43 version — do NOT bump the version to "unlock" styling), **null `style` on base + all breakpoints** and re-express the needed keys as designer props / breakpoint blocks.
