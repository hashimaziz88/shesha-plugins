...
2. Every `entityType` (a **full-class-name string** on 0.43 — on `datatableContext`, `autocomplete`, `entityPicker` alike; the 0.45 `{name,module}` object 400s the metadata fetch on 0.43) → target entity.
...
8. `uniqueStateId` / `componentName` on the `datatableContext` — give each table a unique id (don't leave `TABLE_VIEW_TEMPLATE_ID`) so multiple tables on a page don't share state.
...
> **0.43 version lifecycle — read before pushing.** These seeds describe copying markup and pushing it, but on 0.43 forms are **versioned ConfigurationItems** — you must not `UpdateMarkup` a **Live** version in place. A brand-new form is `Create → UpdateMarkup → UpdateStatus 1→2→3`; **editing an existing Live form** is `CreateNewVersion{id} → UpdateMarkup on the NEW Draft → UpdateStatus 1→2→3` (auto-retires the old Live). Follow the full algorithm before any push: [version lifecycle](version-lifecycle.md).
...
| **Card list** (datalist) | `assets/examples/entity-datalist.json` **+** `assets/examples/entity-card.json` | "**list** of X", "cards", "feed", "tiles", "gallery", "directory" — repeating card view; multi-select via `selectionMode: "multiple"`. Copy **BOTH**: the list (`datatableContext` → `datalist`) and its **row-template card form**, then point the datalist's `formId` at your card form. (Live-verified row-template mode — see [data-tables.md](components/data-tables.md). Do NOT use inline `items`; it renders blank on 0.43.x.) |
...
- The employee seeds demonstrate **FK child tables**: `employee-detail-with-child-tables` filters an **Entity-sourced** `datatableContext` with a `permanentFilter` on the child's `<parentFk>` (shape below).
- **M:M junction subtables are NOT in this seed set** — they use the **Url-sourced** `datatableContext` canon (code-object `endpoint` returning a `/api/dynamic/<module>/<Junction>/Crud/GetAll?filter=...` URL): see [components/junction-subtables.md](components/junction-subtables.md).
...
4. **Child tables** live in a `tabs` component; each tab is a `datatableContext` + `datatable` filtered to the parent.
...
"onSuccess": {
  ...
  "actionOwner": "<datatableContext component id>"
}
```

`actionOwner` must be the table's `datatableContext` **component id** (the same owner the toolbar Refresh button uses). Keep `handleFail: true` + `onFail: Close Dialog`.

## Non-obvious specifics the examples encode

- **Data context type is `datatableContext`** (canonical on 0.43, version 8 here — probe yours) with `sourceType: "Entity"`, `entityType: "<full.Class.Name>"`, `dataFetchingMode: "paging"`, `defaultPageSize: 10`, `uniqueStateId`, `componentName`, `propertyName`, `sortMode: "standard"`, `allowReordering: "no"`. (0.43 has **no** `dataContext` component — a `dataContext` renders an empty data area; always author `datatableContext`.)
- **Toolbar buttons are context-scoped**: Refresh = `actionName: "Refresh table"`, column toggle = `"Toggle Columns Selector"`, both with `actionOwner` set to the **datatableContext component's id** (not `shesha.common`).
