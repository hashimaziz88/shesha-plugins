...
- For a plain URL instead: `"actionArguments": { "navigationType": "url", "url": "/settings/theme/", "queryParameters": [] }` (used by built-in items like Theme and the `/settings` pages).
