...
| Push | `curl --data-binary @req.json` or `Invoke-RestMethod -InFile req.json`. **Never inline PowerShell bodies** — encoding + size traps (`Invoke-WebRequest -Body` can prompt for credentials in NonInteractive mode on large payloads). **0.43: do NOT `UpdateMarkup` a Live version in place** — per target, resolve the version and run `CreateNewVersion{id} → UpdateMarkup(new Draft) → UpdateStatus 1→2→3` (or reuse an in-flight non-Live Draft); see the lifecycle caveat below. |
...
## 0.43 version lifecycle at fleet scale (do NOT clobber Live forms)

On 0.43 every form is a **versioned ConfigurationItem** — a fleet transform that `UpdateMarkup`s the resolved id **in place clobbers the Live version** and loses its history. Per target, the push step is not one call but the lifecycle cycle:

- **Resolve** each target's version id + versionNo + versionStatus + Origin first (part of the Step-1 audit).
- **Live(3):** `CreateNewVersion{id}` → `UpdateMarkup` on the **new Draft** id (never the Live id) → `UpdateStatus 1→2→3` (auto-retires the old Live).
- **Draft(1)/Ready(2) already in flight:** reuse the newest non-Live version — `UpdateMarkup` on it, publish when the transform passes its assertions.
- **Retired(5)/Cancelled(4):** never edit; resolve to the latest non-terminal version first.

Invariant per target: `CreateNewVersion` at most **once** per target per rollout; every re-push (verify/fix loop) targets the **same Draft**; publish once. A fleet script must carry the version id it created and re-use it — a second `CreateNewVersion` on a re-run forks a duplicate Draft. Full algorithm + failure recovery: [version lifecycle](version-lifecycle.md).
...
- **Preserve identity + data keys**: `id`, `parentId`, `componentName`, `propertyName`, `version`, plus data bindings (`referenceListId`, `entityType`, bound `content`). **0.43 caveat:** only preserve `version` when template and targets are the **same stack version**. A template captured from a 0.45 form carries 0.45 component versions (e.g. `container` 7, `datatable` 29, `buttonGroup` 15) — transplanting those onto a 0.43 form ships wrong versions that render blank/stale. Author component versions from a **runtime version-probe** of the live 0.43 backend (max version per component type from `FormConfiguration/GetAll`), not from the template's `version` field.
