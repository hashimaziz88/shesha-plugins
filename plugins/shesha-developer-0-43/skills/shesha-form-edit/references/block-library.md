...
| `rail-panel` | list | A count-badged rail card for a linked collection: title + count + inline `+` add link over a datalist bound to its own `datatableContext`. | `title.text`, `count.badge.text`, `add.button.label`, `datalist` | datatableContext entityType/endpoint, datalist `formId`, add `formId`, onSuccess owner |
...
- `$MODULE` / `$ENTITY` / `$OWNERFK` / `$PROJECTION` — substitutions in datatableContext endpoint code.
...
4. **Fill `$bindings`**: walk the block's `$bindings` list and write each `$binding:` placeholder —
   ...
   labels, count/body content expressions, datatableContext endpoints, dialog `formId`s, and the
   `onSuccess.actionOwner` that must equal the owning datatableContext/datalist id.
