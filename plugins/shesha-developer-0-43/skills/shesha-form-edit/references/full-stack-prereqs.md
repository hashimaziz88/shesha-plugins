...
**Handoff (attribute missing / no items / new reflist needed):** `Skill(skill="shesha-developer-0-43:domain-model")`
...
**Handoff:** `Skill(skill="shesha-developer-0-43:domain-model")` — then re-test the dialog (see [add-dialogs.md](components/add-dialogs.md)).
...
|---|---|---|
| HTTP **400** on a `datatable`/`datatableContext` data load | Entity has **no GQL query API enabled** | `Skill(skill="shesha-developer-0-43:domain-model")`; stop-gap: switch the context to `sourceType: "Url"` (see [components/data-tables.md](components/data-tables.md)) |
| `Invalid object name 'Frwk_ConfigurationItems'` on startup/load | **Framework migrations not run** (e.g. DB restored from an older backup) | Run pending FluentMigrator migrations — `Skill(skill="shesha-developer-0-43:domain-model")` |
| NHibernate `could not execute batch command [SQL: SQL not available]` on save | **Entity mapping bugs** (fluent FK mappings, reflist declarations) | `Skill(skill="shesha-developer-0-43:domain-model")` |
...
**Diagnosis-first rule:** run `Skill(skill="shesha-developer-0-43:test-entity-crud-api", args="--no-fix")` to enumerate which entities' GET endpoints are broken **before** changing anything — a form bound to a broken entity looks fine in markup and fails only at runtime.
...
**Handoff (writing the handlers):** `Skill(skill="shesha-developer-0-43:shesha-app-layer")` for the application-layer pieces; `Skill(skill="shesha-developer-0-43:domain-model")` if entities/migrations change.
...
For **new entity-bound forms** or any entity you have not verified this session, dispatch the `shesha-developer-0-43:fullstack-prereq-checker` agent (via the Task tool) **before authoring** — it front-loads sections 1–3 (entity exists, metadata resolves, reflists populated, CRUD endpoints answer) instead of you discovering them one runtime failure at a time. See SKILL.md **Step 4.5** for the inline fallback (entity existence check via `Metadata/GetProperties`, `modelType` resolution, `test-entity-crud-api --no-fix`).

**0.43 — the prereq gate is about the backend; the push still follows the version lifecycle.** Passing the
prereq check means the entity/reflist/CRUD are ready; it does **not** exempt the form push from versioning.
Once prereqs are green, author and push per the lifecycle: a brand-new form is Created as a Draft(1) and must
be published (`UpdateStatus` 1→2→3) before it renders; editing an existing Live form needs `CreateNewVersion`
first. This applies per form when a prereq-gated fleet authors many entity-bound forms — each push branches
(if Live) / reuses an in-flight Draft, then publishes once. See [version lifecycle](version-lifecycle.md#edit-algorithm) and [api.md §4.5](api.md).
