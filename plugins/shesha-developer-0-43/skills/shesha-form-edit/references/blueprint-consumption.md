...
When this skill is invoked by `shesha-claude-designer` (or directly with a design to match), the requirements arrive as a **layout blueprint** — a `<screen>.blueprint.md` produced by `shesha-developer-0-43:shesha-design-comprehension`. The blueprint is a *measured placement contract*; build to it exactly, then expect a placement re-measure (gate 5a.5) against its `assertions`.
