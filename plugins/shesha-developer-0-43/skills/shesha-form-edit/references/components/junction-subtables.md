...
**Scope:** this EXTENDS [child-tables.md](child-tables.md). That file covers the simple FK-child pattern (`sourceType: "Entity"` + `permanentFilter` on the child's FK). This file covers tabs bound to an M:M **junction entity** via a Url-sourced `datatableContext` — link/unlink rows, dot-notation columns, drill-down through the junction to the far entity. Wrapper rules and inner components: [data-tables.md](data-tables.md). Add/link dialogs: [add-dialogs.md](add-dialogs.md).
...
The tab's **ONLY direct child is the `datatableContext`**; toolbar / add-button wrapper / datatable are three siblings INSIDE it:

```
tab
└─ datatableContext            sourceType:"Url", entityType:"<Full.Junction.ClassName>",
...
```json
{
  ...
  "sourceType": "Url",
  "entityType": "<Full.Junction.ClassName>",
  "endpoint": {
    "_mode": "code",
...
- `entityType` on 0.43 is the **full-class-name string** everywhere (subtables AND standalone tables) — e.g. `"Shesha.RequirementsStudio.ModuleDefinitionUsedModule"`. The 0.45 `{name, module}` object form makes 0.43's metadata fetch 400 and the table renders empty. (`formId` on navigation/dialogs is still `{name, module}` — that's unrelated.)
...
1. the `datatableContext.endpoint` filter (`<parentFk>.id == data.id`),
...
```json
{
  ...
  "actionConfiguration": {
    ...
    "onSuccess": { "actionName": "Refresh table", "actionOwner": "<enclosing datatableContext id>" }
  }
}
```

- `<Entity>` = the enclosing `datatableContext.entityType` (the junction on M:M tabs).
- Resolve the Refresh owner by **walking the datatable's parent chain to the first `datatableContext`/`datatableContext`** — NOT `datatable.parentId`, which is a container on table pages.
