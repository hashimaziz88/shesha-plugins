...
> `KeyInformationBar` `version` is **probe-resolve** on 0.43 — census the running backend for its live
> version rather than hardcoding; if the backend has no form using it, clone the value from the
> designer's own output.
