...
└─ tabs  (componentName: "childTables")
   └─ tab (title: "<Children>", e.g. "Payslips")
      └─ datatableContext  (componentName: "childTableContext")
...
Add more `tab` entries to the same `tabs` component — each with its own `datatableContext` + `datatable` + `permanentFilter` for that child entity. One tab per related entity.

## Gotchas

- The child `datatableContext` needs its **own** `uniqueStateId` / `componentName` (don't reuse the main form's).
