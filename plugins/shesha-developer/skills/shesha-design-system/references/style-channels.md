...
Fix (style channel — this skill's half): null `style` on base + desktop + tablet + mobile of `container60` (designer props already carried bg/border/width). Banner `style` strings identical across all 17 forms (`flex: '1 1 auto'…`) were left alone. The other half of this fix is structural, not a style-channel matter — deleting the `container52` wrapper and re-parenting the KIB container directly into `container60` — and is documented in `shesha-form-edit` → `components/detail-page-pattern.md` ("Known parasites fixed during normalization"), moved there in Task 4 (2026-07-30) rather than restated here.
