...
### Worked example — legacy `style` masking `alignItems`

A header band showed dead space above a KIB strip and a title indent no other form had. `alignItems: "stretch"` was stamped on base + desktop + tablet + mobile; the JSON looked right; the DOM rendered `flex-start` plus unexplained padding. The stylingBox audit showed the band clean (`stylingBox: "{}"`).

Cause: channel 4. The container carried a legacy `style` string — `"return { padding: '10px 0px 10px 25px', alignItems: 'flex-start' }"` — rendered inline, overriding everything; and a KIB wrapper container carried `style` with `flex: '0 0 auto'`, `alignItems: 'flex-end'`, `alignSelf: 'flex-start'`, blocking the vertical stretch. The template form had `style: null` on the analogous containers — which is exactly why only the affected forms misbehaved.

Fix (style channel — this skill's half): null `style` on base + desktop + tablet + mobile of the container (designer props already carried bg/border/width). `style` strings identical across all forms including the template (e.g. a banner's `flex: '1 1 auto'…`) were left alone. The other half of this fix is structural, not a style-channel matter — deleting the KIB wrapper and re-parenting the KIB container directly into the header band — and is documented in `shesha-form-edit` → `components/detail-page-pattern.md` ("Known parasites fixed during normalization") rather than restated here.
