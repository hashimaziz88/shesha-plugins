...
- **Construction mapping for the title/header `text` a form always carries** (moved from `shesha-form-edit/references/form-quality.md` — Task 4, 2026-07-30; `shesha-form-edit` mandates only that every title/header sets `fontSize`+`fontWeight` explicitly, this file owns the values): page title `text-2xl` / **24** / **600** (**700** only if the design's title is genuinely bold); card header `text-base` / **16** / **600**; section header `text-sm` / **13–14** / **600**. This is the same scale-by-role rule above, restated as the exact Tailwind-class/px/weight triad the compiler/authors use day to day. The header's *surface* treatment (band background, bottom hairline) is the `page-title-band` recipe in [component-recipes.md](component-recipes.md).
