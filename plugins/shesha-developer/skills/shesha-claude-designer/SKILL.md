...
description: THE ENTRY POINT for building a multi-screen Shesha UI that must look properly designed — with or without a design source. Use when the user hands over a design to MATCH (prototype, HTML/JSX, Figma-style kit, screenshots) or only describes it in prose. Triggers: "build this design in Shesha", "turn this prototype into Shesha forms", "create an X system with N pages", "make it look properly designed, not default AntD". It conducts across screens — a blueprint each — then orchestrates shesha-design-comprehension, shesha-form-edit and shesha-design-system and verifies by measurement. For a SINGLE form with no design intent use shesha-form-edit; to restyle one working form use shesha-design-system.
---

# Shesha Claude Designer

**The conductor for design → on-brand Shesha app.** It turns each screen into a **measured layout blueprint**, plans the screens, then delegates and verifies: structure to `shesha-form-edit`, styling to `shesha-design-system`, comprehension + placement to `shesha-design-comprehension`.

## Step R — route before paying for the pipeline

| Task shape | Route |
|---|---|
| Single screen, no design intent ("add a checkbox to X") | Hand the whole task to `Skill(shesha-developer:shesha-form-edit)` and stop. |
...
2. **Screen inventory: read it out of the brief** — each named screen → a row (name, archetype from `shesha-form-edit/references/archetypes.md`, entity, cross-links). "A bookings list, a create dialog and a details page" → `list-card`, `capture`, `record-detail`. Write to `$RUN_DIR/screen-inventory.json`.
...
- **5a — Structural integrity:** archetype built with native components, fully flexed, fields bound. Failures route back to `shesha-form-edit`.
- **5a.5 — PLACEMENT diff (REQUIRED `shesha-design-comprehension`):** re-probe the built, published, table→details form into `$RUN_DIR/probes/` and diff against the blueprint `assertions`; route mismatches back to `shesha-form-edit`. **On Tier D, say what it did and did not prove** — it matches the archetype's structure, not a user design.
...
- **Delegate ownership, one push path:** this skill plans, sequences and gates but never authors JSON, picks hexes, or pushes — all writes go through `shesha-form-edit`. Read the source, not the bundle; be honest about gaps rather than claim an unachievable pixel match.
