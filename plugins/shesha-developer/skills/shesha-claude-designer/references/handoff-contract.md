...
6. **Multi-screen** — `shesha-spec` may dispatch one form-author per distinct new form; comprehension verifies each screen's placement independently. **After every dispatch, verify the artifact on disk before accepting the agent's verdict** (see Contract A).
