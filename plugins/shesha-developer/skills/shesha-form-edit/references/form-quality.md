...
- **Appearance floor (presence check)** — markers + the full type scale live in `shesha-design-system/references/default-theme-quickpass.md` (single source, do not restate values here). HOW the floor gets applied is that file's job; THAT it happened is this construction check. Exempt only if a brand/blueprint pipeline styled it.
...
- **Titles carry explicit `fontSize`+`fontWeight`** sized to role per the type scale in `shesha-design-system/references/default-theme-quickpass.md`. Unstyled headers render at body size and the page loses its hierarchy.
