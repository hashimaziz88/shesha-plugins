...
The self-referential `FacilityLinkedFacility` junction (module `His.Facilities`, accessors `facility` / `linkedFacility`):

- **"Linked Facilities" tab on `facility-details`** (forward/owned side): `dataContext` `entityType: {name: "FacilityLinkedFacility", module: "His.Facilities"}`, endpoint `_code` returns `'/api/dynamic/His.Facilities/FacilityLinkedFacility/Crud/GetAll?filter=' + ...` filtering `facility.id == data.id`. Columns: `linkedFacility.name`, `linkedFacility.status` (badge via the referral-status reflist). Eye column → `formId: {name: "facility-details", module: "His.Facilities"}` + `{{selectedRow.linkedFacility.id}}` (`linkedFacility` resolves to `Facility` — a known accessor≠entity exception). Add button → `Show Dialog` on `facility-linked-facility-add`, which needs the gotcha-#7 `onPrepareSubmitData` parent-FK injection ([add-dialogs.md](add-dialogs.md)). Delete column unlinks via `http.delete('/api/dynamic/His.Facilities/FacilityLinkedFacility/Crud/Delete?id=' + selectedRow.id)`.
- **Mirrored "Linked By Facilities" tab** (reverse side, clone-and-swap): endpoint filters `linkedFacility.id == data.id`, columns `facility.name`/`facility.status`, eye → `{{selectedRow.facility.id}}` (same detail form — self-referential). Title starts with "Linked By" → view-only: **no Add, no delete**.
