...
For **new entity-bound forms** or any entity you have not verified this session, run `scripts/backend-probe.mjs` **before authoring** — one call front-loads sections 1–3 (entity exists, metadata resolves, reflists populated, CRUD endpoints answer, permissions allow the operation) instead of discovering them one runtime failure at a time. It exits 1 listing each blocker with the skill that fixes it. For deeper diagnosis of a failing entity, `Skill(shesha-developer:test-entity-crud-api, "--no-fix")`.
