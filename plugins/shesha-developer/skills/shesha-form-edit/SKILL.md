...
> **Building a form to match a design?** If the requirements arrive as a **layout blueprint** (`<screen>.blueprint.json` from `shesha-developer:shesha-design-comprehension`, usually via the `shesha-claude-designer` orchestrator), this is a **compile, not an authoring task** — run `node scripts/compile-spec.mjs <screen>.blueprint.json --out <form>.json` then `node scripts/validate-form.mjs <form>.json`. No hand-translation of `layout-tree`/`bindings` into components: the compiler resolves the archetype's seed shape, the flex-container splits, the `parentId`s, and the `propertyName`s from the blueprint directly. Full contract (what it owns, what it doesn't, the acceptance property): [references/compiling.md](references/compiling.md). Then expect a placement re-measure — the orchestrator's gate 5a.5, now `verify-placement.mjs`'s exit code — against the blueprint's `assertions`. Field-mapping reference / troubleshooting a compile failure: [references/blueprint-consumption.md](references/blueprint-consumption.md). (The retired `compile-blueprint.js` / Markdown+twin blueprint format is documented in `../shesha-claude-designer/README.md`'s "One compiler, two open questions" section; it is not the build path.)
...
```
node scripts/compile-spec.mjs <bp.json> --out <workdir>/<form>.json
```

The blueprint's own `theme` id selects the brand whose tokens get baked in
[R-042]; `report.theme` names the brand actually used. The compiler types the
JSON: flex containers with `desktop.dimensions.width`
...
| Ground-truth rerun (regenerate the KB/schema/measured-capability-matrix) | [references/gym.md](references/gym.md) |
