...
description: Create and edit Shesha 0.45 form configurations via the API — a compiler pipeline, not a prompt pipeline. Compiles a blueprint IR (or a spec synthesized from prose requirements) into markup through golden archetypes and the measured capability matrix, gates it mechanically (schema → guardrails → bindings → styled-ness), pushes via Create/UpdateMarkup, and verifies the deliverable (re-fetch diff + render instrument). Use when the user provides a form id (or module + name) and requirements like "add a sector dropdown above the email field", "wire the Save button", or "create a new asset worklist". 0.45-only — versioned 0.43-class backends belong to the shesha-developer-0-43 plugin. Ends every new no-design form with the mandatory default `shesha` theme pass via shesha-design-system — no form ships unstyled.
allowed-tools:
  - Bash
  - PowerShell
  - Read
