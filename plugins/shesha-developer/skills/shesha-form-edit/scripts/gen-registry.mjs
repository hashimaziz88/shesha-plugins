 ...
 *   node scripts/gen-registry.mjs --print-source-hash   # print the KB sourceHash, generate nothing
 *
 * OWNERSHIP. assets/components-kb/ is the canonical, hand-inspectable resource;
 * assets/component-registry.json is a GENERATED aggregate of it and is never
 * hand-edited. Every run stamps a `provenance` block (generator + generatorVersion +
 * mode + sourceHash + generatedAt); tests/registry.test.mjs recomputes the sourceHash
 * from the KB and FAILS when it no longer matches the committed registry, which is how
 * "the registry is stale" becomes a test failure rather than a silent lie.
 *
...
/**
 * Bumped by hand whenever the SHAPE this script emits changes (a new field, a
 * different typing rule). It is provenance, not a semver contract: it lets a
 * consumer tell "generated by the pre-provenance generator" from "generated by
 * this one" without diffing 14k lines.
 */
const GENERATOR_VERSION = '1.1.0';

// ---- sourceHash: what "stale" means ------------------------------------------
/**
 * sha256 over the newline-joined, filename-sorted list of
 *   "<filename>:sha256(<file bytes>)"
 * for every NON-underscore *.json in assets/components-kb/.
 *
 * Two deliberate choices, because they define staleness:
 *
 *   1. PER-FILE DIGESTS, not one hash over concatenated bytes. A rename that
 *      preserves content still changes the hash (the filename is inside the
 *      digested line), and the sorted list makes the result independent of
 *      readdir order across platforms.
 *
 *   2. THE UNDERSCORE FILES ARE EXCLUDED — `_index.json`, `_meta.json`,
 *      `_enums.json`, `_gaps.json`. Those are themselves generated side-artifacts
 *      of the KB build with their own edit lifecycles: item 16c re-indexed
 *      `_index.json` alone (orphan indexing) without touching a single component
 *      file, and that must NOT read as "the registry is stale" — no typed prop
 *      the registry emits changed. The hash therefore tracks the COMPONENT
 *      TRUTH (121 per-component files: settingsFields, resolvedProps,
 *      appearanceFieldPaths, slots), which is what actually feeds every row.
 *
 *      The cost, stated honestly: an `_enums.json` refresh changes prop TYPES
 *      (enum members) without moving this hash, so an enum-only regeneration is
 *      not caught by the staleness test. That gap is covered instead by the
 *      release procedure in references/gym.md, which regenerates the registry
 *      immediately after extract-enums.js, and by tests/registry.test.mjs's
 *      measured-vs-registry cross-check.
 */
export function kbSourceHash(kbDir = KB_DIR) {
  const files = fs.readdirSync(kbDir)
    .filter((f) => f.endsWith('.json') && !f.startsWith('_'))
    .sort();
  const lines = files.map((f) => {
    const digest = crypto.createHash('sha256').update(fs.readFileSync(path.join(kbDir, f))).digest('hex');
    return `${f}:${digest}`;
  });
  return {
    sourceHash: `sha256:${crypto.createHash('sha256').update(lines.join('\n')).digest('hex')}`,
...
// --print-source-hash exits before anything is read or generated, so the staleness
// test can ask THIS script for the hash instead of reimplementing the definition.
if (args.includes('--print-source-hash')) {
  const { sourceHash, fileCount } = kbSourceHash();
  console.log(sourceHash);
  console.error(`gen-registry: hashed ${fileCount} components-kb component files (underscore files excluded by design)`);
  process.exit(0);
}

const { GymApi } = await import('./gym-lib/api.js');
...
const { sourceHash, fileCount: kbFileCount } = kbSourceHash();

const registry = {
  // OWNERSHIP STAMP. components-kb is canonical; this file is a generated aggregate,
  // never hand-edited. tests/registry.test.mjs recomputes sourceHash and fails when
  // the KB has moved on — see kbSourceHash() for exactly what is hashed and why.
  provenance: {
    generatedBy: 'gen-registry.mjs',
    generatorVersion: GENERATOR_VERSION,
    mode,
    sourceHash,
    sourceHashScope: `sha256 of sorted "<file>:sha256(content)" over ${kbFileCount} non-underscore assets/components-kb/*.json`,
...
console.log(`gen-registry: mode=${mode}${live ? ` backend=${live.baseUrl}` : ''} generatorVersion=${GENERATOR_VERSION}`);
console.log(`gen-registry: sourceHash=${sourceHash} (${kbFileCount} components-kb component files)`);
