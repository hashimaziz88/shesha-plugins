...
// Read scripts/lib/cluster.mjs's source and strip its ESM `export `
// keywords so the function declarations can live inside PROBE_FN's body
// (a plain, non-module function scope). No other rewriting is needed —
// the module is deliberately written in var/function-expression ES5 style
// for exactly this reason (see its own header comment).
function loadClusterSource() {
  var file = path.join(__dirname, 'lib', 'cluster.mjs');
