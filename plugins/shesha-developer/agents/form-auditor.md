...
- **appearance** — the appearance floor for forms not styled by a brand/blueprint pipeline: page-root container carries the canvas `background.color` (`#F8F8F9` for the default `shesha` theme), section/card containers carry a white background + hairline border + radius, titled header `text` components carry explicit `desktop.font.size`+`desktop.font.weight` (nested style paths — there is no flat `fontSize`/`fontWeight` prop [components-kb.md, registry-0.45.1.json]), and each action `buttonGroup` has exactly one `buttonType:"primary"`. An all-default unstyled tree (no background, no borders, unsized titles) = `fail`. Expected values: `shesha-design-system/references/default-theme-quickpass.md` (sibling skill of SKILL_ROOT). If the dispatch prompt states the form was styled against a named brand/blueprint, check for THAT brand's markers instead of the default's.
