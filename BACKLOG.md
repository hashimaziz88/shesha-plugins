...
| BL-014 | Property-based compiler fuzzer: generate random valid SFS documents from `sfs.schema.json` and assert the section 2.6 predicates plus Q5 determinism on every one, so the invariants are tested over a generated population rather than over the declared fixtures alone | section 2.4 | WP-1.a | No | `node --test packages/sfs/test/fuzz.test.mjs` -> exit 0 with `cases >= 200` and 0 predicate failures |
| BL-015 | Keep WP-1.a's second compiler as a permanent differential oracle: promote `tools/normalise-legacy.mjs` to a retained comparison arm, run P3 over the whole declared subset in CI, and add a `.github/workflows/` job whose only step is `npm run green` on push and pull_request | section 2.4.5 | WP-1.a | No | `node --test packages/sfs/test/oracle.test.mjs` -> exit 0 over 6 forms, and the workflow file runs `npm run green` with no `continue-on-error` |
| BL-020 | Registry `propsCompleteness: full >= 93 of 121`: parse EVERY component's TS interface at the pinned framework commit, not only the 13 priority types WP-2 does. §2.8's alias for BL-004; both name the same all-121 source-parse | §2.8.4 | WP-2 | No | `node packages/registry/src/validate.mjs` -> `full >= 93/121` |
| BL-022 | Establish framework versions for the 8 authorable-but-version-unknown types (`buttons`, `dynamicView`, `imagePicker`, `logViewer`, `paragraph`, `permissionTagGroup`, `processMonitor`, `threeStateSwitch`) and flip each from `authorable:false` (D-114) to authorable, lowering `deferredAuthorable` | §2.8.4 | WP-2 | No | `node packages/registry/src/validate.mjs` -> `deferredAuthorable < 8` and every flipped type has a non-null version |
| GAP-001 | DONE (WP-6, D-111): the action grammar gained `openDialog` argIgnore + `closeDialog`/`confirm`/`startEdit`/`cancelEdit` intents, so production dialog/edit/column configs lift; employee-table, employee-create and rs-table moved to declaredSubset clean, rate stayed 1.00 | §2.1.7 | WP-5.b | No | `npm run sfs -- roundtrip --scope packages/sfs/config/roundtrip-expected.json` with employee-table MOVED to declaredSubset clean -> rate stays >= 0.90 |
...
| BL-024 | Lift the remaining escaping constructs to SFS. `buttonGroup`/action-grammar DONE (WP-6, D-111). The three the four still-triaged corpus forms need are the container/leaf node-types the compiler does not yet emit: `sectionSeparator` (rs-create-dialog, rs-detail-with-header), `collapsiblePanel`->`panel` (both employee-detail forms), `tabs` (rs-detail-with-header, employee-detail-with-child-tables) — each needs an `sfsNode` overlay + compiler expansion + decompiler lift; `tabs` also feeds T3's tabKey. Non-corpus constructs stay backlog: `statusTag`, `title`, `button`, `alert`, `wizard`, `htmlRender`, and UNEQUAL-span `columns` grids (D-101 lifts only equal spans) | §2.5 MINING-REPORT.md §5 | WP-5e | No | `npm run sfs -- roundtrip` over a form using each construct -> that construct lifts and contributes zero structural escapes |
